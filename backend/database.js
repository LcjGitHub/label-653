...
function seedDefaultData(resolve, reject) {
...
function seedArticles(resolve, reject) {
...
function checkAndAddColumns(resolve, reject) {
  db.all("PRAGMA table_info(articles)", [], (err, columns) => {
    ...
    function checkDone() {
      if (pendingAlters === 0) {
        checkAndAddUserColumns(resolve, reject);
      }
    }

    if (!hasCategoryId) {
      pendingAlters++;
      db.run(`ALTER TABLE articles ADD COLUMN category_id INTEGER`, (err) => {
        if (err) {
    ...
    if (!hasPinnedAt) {
      pendingAlters++;
      db.run(`ALTER TABLE articles ADD COLUMN pinned_at DATETIME`, (err) => {
        if (err) {
...
function checkAndAddUserColumns(resolve, reject) {
  const tables = ['likes', 'favorites', 'comments'];
  let pendingChecks = tables.length;
  let allDone = false;

  function checkAllDone() {
    pendingChecks--;
    if (pendingChecks <= 0 && !allDone) {
      allDone = true;
      seedArticles(resolve, reject);
    }
  }
  ...
  tables.forEach(table => {
    db.all(`PRAGMA table_info(${table})`, [], (err, columns) => {
      if (err) {
        console.warn(`检查表 ${table} 结构失败:`, err.message);
        checkAllDone();
        return;
      }
      ...
      if (!hasUserId) {
        db.run(`ALTER TABLE ${table} ADD COLUMN user_id INTEGER REFERENCES users(id) ON DELETE SET NULL`, (alterErr) => {
          if (alterErr) {
            console.warn(`为 ${table} 表添加 user_id 列失败:`, alterErr.message);
          } else {
            console.log(`已为 ${table} 表添加 user_id 列`);
          }
          checkAllDone();
        });
      } else {
